...
LANG_TEXTS = {
    'ru': {
        'welcome'       : '👋 *Добро пожаловать в RAVONX MARKET!*',
        'share_prompt'  : '━━━━━━━━━━━━━━━━━━\nДля доступа ко всем функциям, пожалуйста, поделитесь контактом.',
        'share_btn'     : '📱 Поделиться контактом',
        'insufficient'  : '❌ *Недостаточно средств!*\n\n💰 Ваш баланс: *{bal} тг*\n💸 Нужно: *{price} тг*\n\n',
        'topup_btn'     : '💳 Пополнить баланс',
        'buy_bal_btn'   : '💰 Купить с баланса ({price} тг)',
        'my_keys'       : '🔑 Мои ключи',
        'no_keys'       : '🔑 *МОИ КЛЮЧИ*\n\n_У вас пока нет купленных ключей._',
        'lang_btn'      : '🇰🇿 Қазақша',
        'status_ok'     : '🟢 Активен',
        'stat_label'    : '📊 СТАТИСТИКА',
        'bal_label'     : '💰 Баланс',
        'spent_label'   : '🛒 Покупок',
        'ref_label'     : '👥 Рефералов',
    },
    'kz': {
        'welcome'       : '👋 *RAVONX MARKET-ке қош келдіңіз!*',
        'share_prompt'  : '━━━━━━━━━━━━━━━━━━\nБарлық функцияларға қол жеткізу үшін контактімен бөлісіңіз.',
        'share_btn'     : '📱 Байланысты бөлісу',
        'insufficient'  : '❌ *Қаражат жеткіліксіз!*\n\n💰 Балансыңыз: *{bal} тг*\n💸 Керек: *{price} тг*\n\n',
        'topup_btn'     : '💳 Баланс толтыру',
        'buy_bal_btn'   : '💰 Балансқа сатып алу ({price} тг)',
        'my_keys'       : '🔑 Менің кілттерім',
        'no_keys'       : '🔑 *МЕНІҢ КІЛТТЕРІМ*\n\n_Сізде әлі сатып алынған кілттер жоқ._',
        'lang_btn'      : '🇷🇺 Русский',
        'status_ok'     : '🟢 Белсенді',
        'stat_label'    : '📊 СТАТИСТИКА',
        'bal_label'     : '💰 Баланс',
        'spent_label'   : '🛒 Сатып алу',
        'ref_label'     : '👥 Рефералдар',
    }
}
...
def main_menu():
    ...
    kb.row(
        types.InlineKeyboardButton("🚀 КУПИТЬ СОФТ", callback_data="cat_mods"),
        types.InlineKeyboardButton("🎮 FREE FIRE", callback_data="cat_accs")
    )
    kb.row(
    ...
    kb.row(
        types.InlineKeyboardButton("📊 СТАТИСТИКА", callback_data="client_stats"),
        types.InlineKeyboardButton("📜 ПРАВИЛА", callback_data="show_rules")
    )
    kb.row(types.InlineKeyboardButton(f"🎁 БОНУС ЗА ПОДПИСКУ (+{SUB_BONUS} тг)", callback_data="sub_check_bonus"))
    kb.row(types.InlineKeyboardButton("🆘 ТЕХ.ПОДДЕРЖКА / ЧАТ", url=CHAT_SUPPORT))
    return kb
...
def send_dashboard(chat_id, user_id):
    """Отправляет красивый дашборд + главное меню."""
    ...
    text  = f"💠 *RAVONX MARKET* 💠\n"
    text += f"━━━━━━━━━━━━━━━━━━\n"
    text += f"👤 User: {uname}\n"
    text += f"🆔 ID: `{user_id}`\n\n"
    text += f"*{t['stat_label']}:*\n"
    text += f"┌ {t['bal_label']}: *{balance} тг*\n"
    text += f"├ {t['spent_label']}: *{orders_cnt}*\n"
    text += f"├ {t['ref_label']}: *{ref_count}*\n"
    text += f"└ 🛡 Статус: {t['status_ok']}\n\n"
    text += f"🕒 Время: *{now_time}*\n"
    text += f"━━━━━━━━━━━━━━━━━━"
    ...
    bot.send_message(chat_id, "🏪 *RAVONX MARKET*", reply_markup=main_menu(), parse_mode="Markdown")
...
def handle_all_callbacks(call):
    ...
    if user_id == ADMIN_ID and (data.startswith("adm_") or data.startswith("ff_currency") or data.startswith("adm_editfield_")):
        ...
        if data == "adm_stats":
            ...
            kb.add(
                ...
                types.InlineKeyboardButton(f"🔑 Ключи: {keys_cnt} шт. | ➕ Добавить", callback_data=f"adm_addkey_{key}"),
                types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="adm_products")
            )
            bot.edit_message_text(
                f"🛍 *{product['name']}*\n💰 Базовая цена: {product['price']} тг{dur_info}\n🔑 Ключей в базе: *{keys_cnt}*\n\nЧто изменить?",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
    ...
    if data == "to_main":
        ...
        if already_verified:
            bot.answer_callback_query(call.id, "✅ Вы уже получили бонус за подписку!", show_alert=True)
            return
        subscribed = check_subscription(user_id)
        if subscribed:
            mark_sub_verified(user_id)
            add_balance(user_id, SUB_BONUS)
            bot.answer_callback_query(call.id, f"🎉 +{SUB_BONUS} тг зачислено на баланс!", show_alert=True)
            new_bal = get_balance(user_id)
            kb = types.InlineKeyboardMarkup()
            kb.row(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="to_main"))
            bot.edit_message_text(
                f"✅ *БОНУС ПОЛУЧЕН!*\n\n"
                f"🎉 Спасибо за подписку на канал!\n"
                f"💰 Вам начислено: *+{SUB_BONUS} тг*\n"
                f"💳 Новый баланс: *{new_bal} тг*\n\n"
                f"🛒 Теперь можешь купить любой товар с баланса!",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
        else:
            kb = types.InlineKeyboardMarkup()
            kb.row(types.InlineKeyboardButton("📢 Подписаться на канал", url=SUBSCRIPTION_CHANNEL_LINK))
            kb.row(types.InlineKeyboardButton("✅ Я подписался", callback_data="sub_check_bonus"))
            kb.row(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="to_main"))
            bot.edit_message_text(
                f"🎁 *БОНУС ЗА ПОДПИСКУ*\n\n"
                f"Подпишись на наш канал и получи *+{SUB_BONUS} тг* на баланс!\n\n"
                f"1️⃣ Нажми «Подписаться на канал»\n"
                f"2️⃣ Подпишись\n"
                f"3️⃣ Нажми «Я подписался»\n\n"
                f"📢 Канал: {SUBSCRIPTION_CHANNEL_LINK}",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
        ...
        kb.row(
            types.InlineKeyboardButton(t['my_keys'], callback_data="my_keys"),
            types.InlineKeyboardButton(t['lang_btn'], callback_data="lang_toggle")
        )
        kb.row(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="to_main"))
        ...
        flag = '🇰🇿 Қазақша' if new_lang == 'kz' else '🇷🇺 Русский'
        bot.answer_callback_query(call.id, f"Язык изменён: {flag}", show_alert=False)
        # Обновляем профиль на новом языке
        orders_cnt = get_user_order_count(user_id)
        ref_count, _ = get_user_referral_info(user_id)
        balance = get_balance(user_id)
        ref_link = f"https://t.me/{BOT_USERNAME}?start=ref_{user_id}"
        ...
        for p_id, p_info in products.items():
            ...
            if durations:
                ...
                btn_text = f"{p_info['name']} — {p_info['price']} KZT"
            kb.add(types.InlineKeyboardButton(btn_text, callback_data=f"buy_{p_id}"))
        kb.add(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="to_main"))
        bot.edit_message_text("🧬 *ВЫБЕРИ СОФТ:*", chat_id, msg_id, reply_markup=kb, parse_mode="Markdown")
        ...
        if durations:
            ...
            for dur_id, label, price in durations:
                kb.add(types.InlineKeyboardButton(f"🛒 {label} — {price} тг", callback_data=f"buyopt_{item_id}_{dur_id}"))
            ...
            if balance >= price:
                ...
                kb.add(types.InlineKeyboardButton(
                    t['buy_bal_btn'].format(price=price),
                    callback_data=f"buywithbal_{item_id}"
                ))
            else:
                t = LANG_TEXTS[get_language(user_id)]
                kb.add(types.InlineKeyboardButton(
                    t['topup_btn'], callback_data="topup_balance"
                ))
            kb.add(types.InlineKeyboardButton("💳 Оплатить через Kaspi", callback_data=f"pay_kaspi_{item_id}"))
            kb.add(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data="cat_mods"))
            bal_info = f"\n💰 Ваш баланс: *{balance} тг*" if balance < price else f"\n💰 Баланс: *{balance} тг* ✅"
            bot.edit_message_text(
                f"🛒 *{product['name']}*\n\n💸 *Цена: {price} тг*{bal_info}\n\n💳 *Выбери способ оплаты:*",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
        ...
        if balance < price:
            ...
            t = LANG_TEXTS[lang]
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton(t['topup_btn'], callback_data="topup_balance"))
            kb.add(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data=f"buy_{item_id}"))
            bot.edit_message_text(
                t['insufficient'].format(bal=balance, price=price) + "Пополни баланс или выбери другой способ.",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
            return
        key_value = pop_product_key(item_id)
        set_balance(user_id, balance - price)
        save_order(user_id, product['name'])
        mark_key_buyer(item_id, user_id)
        if key_value:
            bot.edit_message_text(
                f"✅ *ПОКУПКА ВЫПОЛНЕНА!*\n\n📦 *{product['name']}*\n"
                f"💸 Списано: *{price} тг*\n💰 Остаток: *{balance - price} тг*\n\n"
            ...
            bot.edit_message_text(
                f"✅ *ПОКУПКА ВЫПОЛНЕНА!*\n\n📦 *{product['name']}*\n"
                f"💸 Списано: *{price} тг*\n\n⚠️ Ключ будет выдан администратором в ближайшее время.",
                chat_id, msg_id, parse_mode="Markdown"
            )
            try:
        ...
        if balance < price:
            ...
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton(t['topup_btn'], callback_data="topup_balance"))
            kb.add(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data=f"buyopt_{item_key}_{dur_id}"))
            bot.edit_message_text(
                t['insufficient'].format(bal=balance, price=price) + "Пополни баланс или выбери другой способ.",
                chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
            )
        ...
        set_balance(user_id, balance - price)
        save_order(user_id, f"{product['name']} ({label})")
        ...
        if balance >= base_price:
            kb.add(types.InlineKeyboardButton(
                t['buy_bal_btn'].format(price=base_price),
                callback_data=f"buywithbaldur_{item_key}_{dur_id}"
            ))
        else:
            kb.add(types.InlineKeyboardButton(t['topup_btn'], callback_data="topup_balance"))
        kb.add(types.InlineKeyboardButton("💳 Оплатить через Kaspi", callback_data=f"paydur_kaspi_{dur_id}_{user_id}"))
        kb.add(types.InlineKeyboardButton("⬅️ НАЗАД", callback_data=f"buy_{item_key}"))
        bal_info = f"\n💰 Баланс: *{balance} тг*" + (" ✅" if balance >= base_price else "")
        bot.edit_message_text(
            f"🛒 *{product['name']}*\n📦 Вариант: *{label}*\n\n💸 *Цена: {base_price} тг*{bal_info}\n\n💳 *Выбери способ оплаты:*",
            chat_id, msg_id, reply_markup=kb, parse_mode="Markdown"
        )
...
def process_addkey(message):
    ...
    total = get_keys_count(product_key)
    bot.send_message(
        ADMIN_ID,
...
def process_balance_edit_step2(message):
    ...
    bot.send_message(
        ADMIN_ID,
        f"✅ Баланс пользователя `{target_uid}` установлен: *{new_balance} тг*",
        reply_markup=admin_menu(),
        parse_mode="Markdown"
    )
...
def process_topup_kaspi_name(message):
    ...
    msg = bot.send_message(
        user_id,
        "📸 *Отправьте скриншот чека* (фото из Kaspi):\n\n"
        "_(Чек должен быть виден полностью)_",
        parse_mode="Markdown"
    )
